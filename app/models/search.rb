class Search
  ...
  class << self
    ...
    def find(query)
      params = query[/([a-zA-Z-]+)\s?/, 1].downcase
      p params
      result = DATA.select {|d| is_include?(d[:Name],params) || is_include?(d[:Type],params) || is_include?(d["Designed by".to_sym],params)}
      if query[/-/]
       negative = query[/.\s-(.+)/,1].downcase
        result = result.reject {|d| is_include?(d[:Name],negative) || is_include?(d[:Type],negative) || is_include?(d["Designed by".to_sym],negative)}
      end
      result.sort_by {|k,v|v}
    end 

    def is_include?(field,params)
      field.downcase.include?(params)
    end
